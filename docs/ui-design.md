...
- **subagent_select**: サブエージェント選択画面（アイコン付き）。
  ...
  Workflow ツール経由の agent (`AgentEntity.workflow_run = Some(..)`) は
  🧩 アイコン + 行頭 `wf:<run>` マーカーで区別し、型カラムには
  `workflow_label`（generic な `workflow-subagent` のみ prompt から導出した
  ロール）があればそれを、無ければ `agent_type` を表示する
